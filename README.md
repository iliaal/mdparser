...
- **`nofollowLinks` is byte-pattern based.** It rewrites every literal `<a href="...">` it finds in the output. The scan emits raw `<script>` and `<style>` regions verbatim and so does not touch anchor-shaped substrings inside JavaScript or CSS. Anchor-shaped substrings inside other raw HTML attribute values (e.g. `<img alt='<a href="x">'>` written directly in the source) are still rewritten and produce malformed HTML; that case lives entirely inside the `unsafe: true` you-own-the-input contract.
