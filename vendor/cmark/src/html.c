...
static int S_render_node(cmark_html_renderer *renderer, cmark_node *node,
  ...
  case CMARK_NODE_STRONG:
    /* cmark upstream (0.31) emits <strong> unconditionally. cmark-gfm
     * added a check to skip the tag when the parent is also a strong,
     * flattening nested bold to a single tag. That matches GitHub's
     * render but diverges from the CommonMark spec, which expects
     * `****foo****` to produce `<strong><strong>foo</strong></strong>`.
     * We follow the spec. */
    if (entering) {
